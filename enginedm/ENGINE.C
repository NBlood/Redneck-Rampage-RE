...
long voxoff[MAXVOXELS][3], voxlock[MAXVOXELS][3];
...
static long spritesx[MAXSPRITESONSCREEN], spritesy[MAXSPRITESONSCREEN];
...
initengine(char davidoption, long daxdim, long daydim)
{
	...
	{
		case 1:
			screen = (char *)kkmalloc((xdim+8)*ydim);
	...
	if ((horizlookup = (long *)kkmalloc(ydim*4*sizeof(long))) == NULL)
		{ printf("OUT OF MEMORY\n"); exit(0); }
	if ((horizlookup2 = (long *)kkmalloc(ydim*4*sizeof(long))) == NULL)
		{ printf("OUT OF MEMORY\n"); exit(0); }
	horizycent = ((ydim*4)>>1);
...
drawmasks()
{
	long i, j, k, l, gap, xs, ys, zs, xp, yp, zp;
		...
		for(i=0;i<spritesortcnt-gap;i++)
			for(l=i;l>=0;l-=gap)
			{
				...
				if (j == 0)
				{
					...
					if (j < 0) break;
					if (j == 0)
						if (klabs(tspriteptr[l]->z-globalposz) < klabs(tspriteptr[l+gap]->z-globalposz))
							break;
				}
...
drawsprite (long snum)
{
	...
	else if ((cstat&48) == 48)
	{
		...
		if ((voxoff[tilenum][0] == 0) || (voxoff[tilenum][1] == 0) || (voxoff[tilenum][2] == 0))
		{
			kloadvoxel(tilenum);
			if (voxoff[tilenum][0] == 0) return;
		}
...
{
	long i, j, k, x, y, z, syoff, ggxstart, ggystart;
	...
	gxstart = ((y*cosang-x*sinang)<<2);
	gystart = ((x*cosang+y*sinang)<<2);
	...
	for(cnt=0;cnt<8;cnt++)
	{
		...
		for(x=xs;x!=xe;x+=xi)
		{
			...
			for(y=ys;y!=ye;y+=yi,nx+=dagyinc,ny-=dagxinc)
			{
				...
				if ((voxptr == voxend) || (ny <= nytooclose)) continue;
	...
	do
	{
		...
		if ((hitwall = raytrace(*x, *y, &intx, &inty)) >= 0)
		{
			...
			for(i=cnt+1;i<=clipmoveboxtracenum;i++)
			{
				...
				if ((templong1^templong2) < 0) return(retval);
			}
...
clearallviews(long dacol)
{
	...
	switch(vidoption)
	{
		...
		case 3:
			for(i=0;i<4;i++)
				{ koutp(0x3cd,i); clearbuf(0xa0000,64000L>>2,0L); }
			break;
		case 4:
			for(i=0;i<4;i++)
				{ koutpw(0x3ce,0x9+(i<<12)); clearbuf(0xa0000,64000L>>2,0L); }
			break;
		case 5:
			for(i=0;i<4;i++)
				{ koutpw(0x3d4,0x35+(i<<8)); clearbuf(0xa0000,64000L>>2,0L); }
			break;
		case 6:
